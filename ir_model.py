...
class ir_model_data(osv.Model):
    ...
    @tools.ormcache()
    def _get_id_via_resid(self, cr, uid, module, model, res_id):
        """
        ...
        """
        ids = self.search(cr, uid, [('module','=',module), ('model','=',model), ('res_id','=', res_id)])
        if not ids:
    ...
    @tools.ormcache()
    def get_object_reference_from_model_resid(self, cr, uid, module, model, res_id):
        """
        Returns (xml_id, ) corresponding to a given model and res_id (cached)
        or raise ValueError if not found
        """
        data_id = self._get_id_via_resid(cr, uid, module, model, res_id)
        res = self.read(cr, uid, data_id, ['xml_id'])
        if not res['xml_id']:
            raise ValueError('No external ID currently defined in the system for: %s.%s' % (model, res_id))
        return (res['xml_id'], )

    def get_object_from_module_model_resid(self, cr, uid, module, model, res_id, context=None):
        """Returns a browsable record for the given model name and res_id or raise ValueError if not found"""
        if context is None:
            context = {}
        id = self._get_id_via_resid(cr, uid, module, model, res_id)
